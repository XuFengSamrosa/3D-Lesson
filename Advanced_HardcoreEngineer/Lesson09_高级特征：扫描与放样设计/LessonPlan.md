...
```text
' 初始化放样曲面矩阵
log_status("Computing Loft Surface Connectors...")
Dim swFeat As Object
Set swFeat = Part.FeatureManager.InsertProtrusionBlend2(False, True, False, 1, 0, 0, 1, 1, True, True, False, 0, 0, 0, True, True, True, swGuideCurveInfluence_e.swGuideCurveInfluenceNextGlobal)
' 校验曲率连续性(C2)
```
...
- 🔴 **[ERR_01] 引导线未与所有截面相交引发的扫描生成中止**
  - **排查链路**：从源头检查几何基准面或传感器原始数据(Raw Data)，切断多余的冗余约束(Redundant Mates/Constraints)。
- 🔴 **[ERR_02] 连接点错位造成的放样曲面自我交叉(Self-intersecting geometry)**
...
**[S-CLASS] 实操考核：**
> 结合3D草图与多个异形截面，建立一个跑车后视镜外壳模型，要求表面必须达到G2级曲率连续。
