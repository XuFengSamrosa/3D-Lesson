...
* **核心目标**：彻底打通【质量属性与材料选择】的底层架构，从原理到工业级落地。

---

## 1. 结构与算法拆解 (Engineering Breakdown)
打通几何拓扑与物理世界，通过材料库注入密度属性，进行惯性张量与力学基础分析。

本节课程的工业级核心节点拆解如下，请务必在实操前深刻理解其数学与物理意义：
1. **密度、泊松比(Poisson's ratio)及屈服强度的物理意义**
   > *深度拓展*：在现代工业标准中，该节点决定了系统的基准误差界限。错误的理解将直接导致后续公差计算(TolAnalyst)与闭环控制系统失控。
2. **质心坐标计算与惯性张量矩阵(Moment of Inertia)**
   > *深度拓展*：掌握此环节可以避免在复杂装配和极限运行状态下的非线性崩溃。
3. **定制化材料库与自定义属性(Custom Properties)注入**
...
```text
' 注入材料属性并计算惯性张量
Part.SetMaterialPropertyName2 "Default", "solidworks materials.sldmat", "Plain Carbon Steel"
Dim massProp As Variant
massProp = Part.Extension.GetMassProperties(1, 0)
log_status("Center of Mass: X=" & massProp(0) & " Y=" & massProp(1) & " Z=" & massProp(2))
```
...
**[S-CLASS] 实操考核：**
> 设计一个陀螺仪转子，通过不断修改不同区域的几何尺寸和掏空设计，使其重心严格位于Z轴，且Z轴转动惯量最大化。
